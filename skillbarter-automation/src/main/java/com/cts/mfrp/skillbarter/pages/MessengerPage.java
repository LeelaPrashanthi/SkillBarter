...
public class MessengerPage {
    ...
    /**
     * Verifies the conversation list rendered with usable content. Intentionally
     * markup-agnostic: we don't bind to a specific child class name (.cname /
     * .clast / .ctime / .cav) because the live build's markup varies and a
     * tighter check produces false-fails when a class is renamed or a brand-
     * new conversation has no message history yet.
     *
     * What we actually require:
     *   1. At least one .contact row is rendered.
     *   2. Every .contact row has SOME non-empty visible text (the contact's
     *      name will always be there even when there's no preview/timestamp).
     *
     * Polls for up to 30s. The row outer <div> mounts before its text hydrates
     * in Angular, so a single-shot sample races with the render cycle. On
     * timeout, every row's text + outerHTML is dumped so the next debug is
     * one log entry away.
     */
    public boolean allConversationsHaveAllFields() {
        final By contactLocator = By.cssSelector("app-chat .contacts .contact");
        try {
            new WebDriverWait(driver, Duration.ofSeconds(30))
                    .until(d -> {
                        List<WebElement> rows = d.findElements(contactLocator);
                        if (rows.isEmpty()) return false;
                        for (WebElement r : rows) {
                            try {
                                if (r.getText().trim().isEmpty()) return false;
                            } catch (Exception e) {
                                return false;
                            }
                        }
            ...
            List<WebElement> rows = driver.findElements(contactLocator);
            log.warn("allConversationsHaveAllFields: timed out after 30s with {} row(s) on screen", rows.size());
            for (int i = 0; i < rows.size(); i++) {
                log.warn("  row #{}: text='{}' outerHTML='{}'",
                        i, safeText(rows.get(i)), safeAttr(rows.get(i), "outerHTML"));
            }
            return false;
        }
    }
    ...
    private String safeAttr(WebElement el, String attr) {
        try { return el.getAttribute(attr); } catch (Exception e) { return ""; }
    }
