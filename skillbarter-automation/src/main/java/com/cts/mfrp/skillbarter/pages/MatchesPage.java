...
public class MatchesPage {
    ...
    /**
     * Clicks "Browse All Users" robustly. The Angular handler doesn't always
     * fire on a plain WebElement.click() — the live page renders the button
     * with mixed inline content (e.g. an icon + text span), so the @FindBy
     * for `button[normalize-space()='Browse All Users']` can fall through to
     * an inert child. We:
     *   1. Locate ANY element in the .hint area whose text contains
     *      "Browse All Users" (button, a, span, div — case-insensitive).
     *   2. Walk up to the nearest clickable ancestor (button / a / [role=button]).
     *   3. Try native click -> Actions click -> JS click in sequence, stopping
     *      when the hint disappears (proof the action fired).
     * Returns true if the hint cleared, false otherwise.
     */
    public boolean clickBrowseAllUsers() {
        WebElement target = findBrowseAllUsersElement();
        if (target == null) {
            log.warn("clickBrowseAllUsers: no element in .hint contains 'Browse All Users'");
            return false;
        }

        // Walk up to the nearest semantically-clickable ancestor so click() lands
        // on the element with the Angular (click) binding, not on an inert child.
        WebElement clickable = walkToClickableAncestor(target);

        JavascriptExecutor js = (JavascriptExecutor) driver;
        try { js.executeScript("arguments[0].scrollIntoView({block:'center'});", clickable); }
        catch (Exception ignored) {}

        // Native click first.
        try { clickable.click(); log.debug("Browse All Users: native click attempted"); }
        catch (Exception e) { log.debug("Browse All Users native click threw: {}", e.getMessage()); }
        if (hintCleared()) return true;

        // Actions click — fires real mouse events that some Angular host listeners require.
        try { new Actions(driver).moveToElement(clickable).pause(java.time.Duration.ofMillis(150)).click().perform();
              log.debug("Browse All Users: Actions click attempted"); }
        catch (Exception e) { log.debug("Browse All Users Actions click threw: {}", e.getMessage()); }
        if (hintCleared()) return true;

        // JS click — last resort.
        try { js.executeScript("arguments[0].click();", clickable);
              log.debug("Browse All Users: JS click attempted"); }
        catch (Exception e) { log.debug("Browse All Users JS click threw: {}", e.getMessage()); }
        return hintCleared();
    }

    /** Permissive locator — matches any element under .hint whose text is exactly "Browse All Users". */
    private WebElement findBrowseAllUsersElement() {
        ...
        try {
            List<WebElement> els = driver.findElements(By.xpath(
                "//div[contains(@class,'hint')]//*[" + tx + "='browse all users']"));
            for (WebElement el : els) {
                try { if (el.isDisplayed()) return el; } catch (Exception ignored) {}
            }
            // Fallback — anywhere on the page, not just inside .hint.
            els = driver.findElements(By.xpath(
                "//*[(self::button or self::a or self::span or self::div) and " + tx + "='browse all users']"));
            for (WebElement el : els) {
                try { if (el.isDisplayed()) return el; } catch (Exception ignored) {}
            }
        } catch (Exception ignored) {}
    ...
    /** Walks up from {@code el} to the nearest <a>/<button>/[role=button] ancestor, falling back to el itself. */
    private WebElement walkToClickableAncestor(WebElement el) {
        try {
            return (WebElement) ((JavascriptExecutor) driver).executeScript(
                "let e = arguments[0];" +
                "while (e && e !== document.body) {" +
                "  const t = e.tagName.toLowerCase(); const r = e.getAttribute('role');" +
                "  if (t === 'a' || t === 'button' || r === 'button' || r === 'menuitem') return e;" +
                "  e = e.parentElement;" +
    ...
    /** True once the .hint container is no longer visible — proof the browse-all action fired. */
    private boolean hintCleared() {
        try {
            new WebDriverWait(driver, java.time.Duration.ofSeconds(3))
                    .until(d -> {
                        List<WebElement> hints = d.findElements(By.xpath("//div[contains(@class,'hint')]"));
                        if (hints.isEmpty()) return true;
                        for (WebElement h : hints) {
                            try { if (h.isDisplayed()) return false; } catch (Exception ignored) {}
                        }
                        return true;
                    });
            return true;
